...
bool ps_init(int brd, int iface)
{
	...
	err = nvs_get_blob(ps_handle, onboarding_details_info_key, NULL, &required_size);
	if ((err != ESP_OK) && (err != ESP_ERR_NVS_NOT_FOUND))
	{
...
static bool ps_read_onboarding_detail_info()
{
	...
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "Get onboarding details info blob failed with %d", err);
		return false;
	}
	if (required_size != sizeof(ps_onboarding_details_t))
	{
		ESP_LOGE(TAG, "Get onboarding details info blob incorrect size %d (expected %d)", required_size, sizeof(ps_onboarding_details_t));
		return false;
	}
...
static bool ps_write_onboarding_details_info()
{
	...
	required_size = sizeof(ps_onboarding_details_t);
	err = nvs_set_blob(ps_handle, onboarding_details_info_key, &ps_onboarding_details_info, required_size);
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "Set onboarding details info blob failed with %d", err);
		return false;
	}
	...
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "Commit onboarding details info failed with %d", err);
		return false;
	}
...
{
	ps_store_string(ps_onboarding_details_info.aws_ip_address, info->aws_ip_address, PS_IP_ADDRESS_MAX_LENGTH);
	ps_onboarding_details_info.port_number = info->port_number;
	ps_store_string(ps_onboarding_details_info.serial_number, info->serial_number, 5);
