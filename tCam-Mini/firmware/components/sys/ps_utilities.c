...
// Uncomment the following directive to force an NVS memory erase (but only do
// it for one execution) #define PS_ERASE_NVS

// Lepton state boolean flags
#define PS_LEP_AGC_EN_MASK 0x01

// Stored Network Flags bitmask
#define PS_NET_FLAG_MASK                                       \
  (NET_INFO_FLAG_STARTUP_ENABLE | NET_INFO_FLAG_CL_STATIC_IP | \
   NET_INFO_FLAG_CLIENT_MODE)
...
//     ap_ssid - Camera name
//     ap_ip_addr - Device address if it is serving DHCP addresses (stand-alone
//     network) stap_ip_addr - Device address when it has a fixed IP address on
//     a network
...
static ps_onboarding_details_t ps_onboarding_details_info;
...
bool is_ps_init_successfully() { return ps_init_success_status; }
/**
...
bool ps_init(int brd, int iface) {
  ...
  //   - Attempt to find the entry in NVS storage and initialize our local copy
  //   from that
  //   - Initialize our local copy with default values and use those to
  //   initialize NVS
  ...
  // Onboarding Details Info
  required_size = 0;
  err = nvs_get_blob(ps_handle, onboarding_details_info_key, NULL,
                     &required_size);
  ESP_LOGE(TAG,
           "NVS get Onboarding Details Info return code %d with message: %s",
           err, esp_err_to_name(err));
  if ((err != ESP_OK) && (err != ESP_ERR_NVS_NOT_FOUND)) {
    ESP_LOGE(TAG,
             "NVS get Onboarding Details Info failed with err code %d with "
             "message: %s",
             err, esp_err_to_name(err));
    return false;
  }
  ...
  ps_init_success_status = success;
  return success;
}
...
static bool ps_read_lep_info() {
  ...
  if (required_size != sizeof(ps_lep_state_t)) {
    ESP_LOGE(TAG, "Get lep info blob incorrect size %d (expected %d)",
             required_size, sizeof(ps_net_info_t));
    return false;
  }
...
static bool ps_read_net_info(int iface) {
  ...
  if (required_size != sizeof(ps_net_info_t)) {
    ESP_LOGE(TAG, "Get network info blob incorrect size %d (expected %d)",
             required_size, sizeof(ps_net_info_t));
    return false;
  }
...
static bool ps_read_old_net_info(int iface) {
  ...
  if (required_size != sizeof(ps_old_net_info_t)) {
    ESP_LOGE(TAG, "Get network old info blob incorrect size %d (expected %d)",
             required_size, sizeof(ps_net_info_t));
    return false;
  }
...
static bool ps_read_onboarding_detail_info() {
  ...
  err = nvs_get_blob(ps_handle, onboarding_details_info_key,
                     &ps_onboarding_details_info, &required_size);
  if (err != ESP_OK) {
    ESP_LOGE(
        TAG,
        "Get onboarding details info blob failed with code %d with message: %s",
        err, esp_err_to_name(err));
    return false;
  }
  if (required_size > sizeof(ps_onboarding_details_t)) {
    ESP_LOGE(TAG,
             "Get onboarding details info blob incorrect size %d (expected %d)",
             required_size, sizeof(ps_onboarding_details_t));
    return false;
  }
  ESP_LOGE(TAG,
           "ps_read_onboarding_detail_info found details: aws_ip_address: %s, "
           "port_number: %d, serial_number: %s, updated_required: %d",
           ps_onboarding_details_info.aws_ip_address,
           ps_onboarding_details_info.port_number,
           ps_onboarding_details_info.serial_number,
           ps_onboarding_details_info.update_required);
...
static bool ps_write_onboarding_details_info() {
  ...
  required_size = sizeof(ps_onboarding_details_t);
  err = nvs_set_blob(ps_handle, onboarding_details_info_key,
                     &ps_onboarding_details_info, required_size);
  if (err != ESP_OK) {
    ESP_LOGE(
        TAG,
        "Set onboarding details info blob failed with code %d with message: %s",
        err, esp_err_to_name(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    ESP_LOGE(
        TAG,
        "Commit onboarding details info failed with code %d with message: %s",
        err, esp_err_to_name(err));
    return false;
  }
...
void ps_set_onboarding_details_info(const onboarding_details_t *info) {
  ESP_LOGE(TAG, "ps_set_onboarding_details_info called");
  ps_store_string(ps_onboarding_details_info.aws_ip_address,
                  info->aws_ip_address, PS_IP_ADDRESS_MAX_LENGTH);
  ps_onboarding_details_info.port_number = info->port_number;
  ps_store_string(ps_onboarding_details_info.serial_number, info->serial_number,
                  5);

  if (!ps_write_onboarding_details_info()) {
    ESP_LOGE(TAG, "Failed to write Onboarding details info to NVS Storage");
  }
}

void ps_get_onboarding_details_info(onboarding_details_t *info) {
  int i;
  ps_onboarding_details_t *local;
  ...
  // Get from our local copy
  ESP_LOGE(TAG, "local aws_ip_address: %s",
           (const char *)local->aws_ip_address);
  strcpy(info->aws_ip_address, (const char *)local->aws_ip_address);
  info->port_number = local->port_number;
  strcpy(info->serial_number, (const char *)local->serial_number);
}

/**
 * Store a string at the specified location in our local buffer making sure it
 * does not exceed the available space and is terminated with a null character.
 */
static void ps_store_string(char *dst, char *src, uint8_t max_len) {
  char c;
  int i = 0;
