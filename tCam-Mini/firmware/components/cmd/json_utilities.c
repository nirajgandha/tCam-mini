 ...
 *
 * Contains functions to generate json text objects and parse text objects into
 * the json objects used by tCam.  Uses the cjson library.  Image data is
 * formatted using Base64 encoding.
 *
 * This module uses two pre-allocated buffers for the json text objects.  One
 * for image data (that can be stored as a file or sent to the host) and one for
 * smaller responses to the host.
 *
 * Be sure to read the requirements about freeing allocated buffers or objects
 * in the function description.  Or BOOM.
 *
...
/**
 * Create a json command object from a string, returns NULL if it fails.  The
 * object will need to be freed using json_free_cmd when it is no longer
 * necessary.
 */
...
/**
 * Update a formatted json string in a pre-allocated json text image buffer
 * containing three json objects for a lepton image buffer.  Returns a non-zero
 * length for a successful operation.
 *   - Image meta-data
...
uint32_t json_get_image_file_string(char *json_image_text,
  ...
  if (success) {
    if (cJSON_PrintPreallocated(root, json_image_text, JSON_MAX_IMAGE_TEXT_LEN,
                                false) == 0) {
      len = 0;
    } else {
...
/**
 * Return a formatted json string containing the camera's operating parameters
 * in response to the get_config commmand.  Include the delimitors since this
 * string will be sent via the socket interface
 */
char *json_get_config(uint32_t *len) {
  ...
  cJSON_AddNumberToObject(config, "agc_enabled",
                          (const double)lep_stP->agc_set_enabled);
  cJSON_AddNumberToObject(config, "emissivity",
                          (const double)lep_stP->emissivity);
  cJSON_AddNumberToObject(config, "gain_mode",
                          (const double)lep_stP->gain_mode);
...
/**
 * Return a formatted json string containing the system status in response to
 * the get_status command.  Include the delimitors since this string will be
 * sent via the socket interface.
 */
char *json_get_status(uint32_t *len) {
  ...
  if (if_type == CTRL_IF_MODE_SIF) {
    // Get the system's default MAC address and add 1 to match the "Soft AP"
    // mode (see "Miscellaneous System APIs" in the ESP-IDF documentation)
  ...
  model_field |= (brd_type == CTRL_BRD_ETH_TYPE) ? CAMERA_MODEL_NUM_ETH
                                                 : CAMERA_MODEL_NUM_WIFI;
  switch (if_type) {
  ...
  sprintf(buf, "%d:%02d:%02d.%d", te.Hour, te.Minute, te.Second,
          te.Millisecond);
  cJSON_AddStringToObject(status, "Time", buf);
  sprintf(buf, "%d/%d/%02d", te.Month, te.Day,
          te.Year - 30);  // Year starts at 1970
...
/**
 * Return a formatted json string containing the wifi setup (minus password) in
 * response to the get_wifi command.  Include the delimitors since this string
...
char *json_get_wifi(uint32_t *len) {
  ...
  sprintf(ip_string, "%d.%d.%d.%d", net_infoP->sta_netmask[3],
          net_infoP->sta_netmask[2], net_infoP->sta_netmask[1],
  ...
  sprintf(ip_string, "%d.%d.%d.%d", net_infoP->cur_ip_addr[3],
          net_infoP->cur_ip_addr[2], net_infoP->cur_ip_addr[1],
...
/**
 * Return a formatted json string containing the cmd and specified length of
 * base64-encoded CCI register data in response to the get_lep_cci/set_lep_cci
 * command.  Include the delimiters since this string will be sent via the
 * socket interface.
 */
char *json_get_cci_response(uint16_t cmd, int cci_len, uint16_t status,
...
/**
 * Return a formatted json string containing the start location and length for a
 * chunk of firmware to return as part of a OTA firmware update.  Include the
 * delimiters since this string will be sent via the socket interface.
 */
...
/**
 * Generate a formatted json string containing the numeric and string info
 * fields.  Add delimiters for transmission over the network.  Returns string
 * length.
 *
 * Note: Because this function is designed to be used by rsp_task, a valid
 * buffer must be passed in for json_string.
 */
int json_get_cam_info(char *json_string, uint32_t info_value,
                      char *info_string) {
  cJSON *root;
  cJSON *response;
  uint32_t len = 0;
...
/**
 * Parse a top level command object, returning the command number and a pointer
 * to a json object containing "args".  The pointer is set to NULL if there are
 * no args.
 */
...
/**
 * Fill in a json_config_t struct with arguments from a set_config command,
 * preserving unmodified elements
 */
bool json_parse_set_config(cJSON *cmd_args, json_config_t *new_st) {
  ...
  if (cmd_args != NULL) {
    if (cJSON_HasObjectItem(cmd_args, "agc_enabled")) {
      new_st->agc_set_enabled =
          cJSON_GetObjectItem(cmd_args, "agc_enabled")->valueint > 0 ? true
                                                                     : false;
      item_count++;
    }
    ...
    if (cJSON_HasObjectItem(cmd_args, "emissivity")) {
      new_st->emissivity =
          cJSON_GetObjectItem(cmd_args, "emissivity")->valueint;
 ...
 */
bool json_parse_set_spotmeter(cJSON *cmd_args, uint16_t *r1, uint16_t *c1,
...
bool json_parse_set_time(cJSON *cmd_args, tmElements_t *te) {
  ...
  if (cmd_args != NULL) {
    ...
    if (cJSON_HasObjectItem(cmd_args, "year")) {
      te->Year =
          cJSON_GetObjectItem(cmd_args, "year")->valueint;  // offset from 1970
      item_count++;
    }
...
/**
 * Fill in a net_info_t object with arguments from a set_wifi command,
 * preserving unmodified elements
 */
  ...
  if (cmd_args != NULL) {
    ...
    if (cJSON_HasObjectItem(cmd_args, "ap_pw")) {
      ...
      if ((i >= 8) && (i <= PS_PW_MAX_LEN)) {
        ...
        ESP_LOGE(TAG, "set_wifi ap_pw: %s must be between 8 and %d characters",
                 s, PS_PW_MAX_LEN);
        return false;
      }
    } else {
    ...
    if (cJSON_HasObjectItem(cmd_args, "sta_pw")) {
      ...
      if ((i >= 8) && (i <= PS_PW_MAX_LEN)) {
        ...
        ESP_LOGE(TAG, "set_wifi sta_pw: %s must be between 8 and %d characters",
                 s, PS_PW_MAX_LEN);
        return false;
      }
    } else {
    ...
    if (cJSON_HasObjectItem(cmd_args, "flags")) {
      new_net_info->flags =
          (uint8_t)cJSON_GetObjectItem(cmd_args, "flags")->valueint;
      item_count++;
    } else {
 ...
 * preserving unmodified elements
 */
bool json_parse_set_onboard_details(
    cJSON *cmd_args, onboarding_details_t *new_onboarding_details_info) {
  char *s;
  int i;
  int item_count = 0;
  ...
  ESP_LOGE(TAG, "json_parse_set_onboard_details called");
  // Get existing settings
  onboarding_details_infoP = get_onboarding_details_info();

  if (cmd_args != NULL) {
    if (cJSON_HasObjectItem(cmd_args, "aws_ip_address")) {
      ...
      strcpy(new_onboarding_details_info->aws_ip_address,
             onboarding_details_infoP->aws_ip_address);
    ...
    if (cJSON_HasObjectItem(cmd_args, "port_number")) {
      ...
      new_onboarding_details_info->port_number =
          onboarding_details_infoP->port_number;
    ...
    if (cJSON_HasObjectItem(cmd_args, "serial_number")) {
      ...
      if ((i >= 1) && (i <= PS_PW_MAX_LEN)) {
        ...
        ESP_LOGE(TAG,
                 "set_onboarding_details serial_number: %s must be between 1 "
                 "and %d characters",
                 s, PS_PW_MAX_LEN);
        return false;
      }
    } else {
      strcpy(new_onboarding_details_info->serial_number,
             onboarding_details_infoP->serial_number);
      serial_number_update_required = true;
    }

    ESP_LOGE(TAG, "%d, %d, %d", aws_ip_update_required,
             port_number_update_required, serial_number_update_required);
    new_onboarding_details_info->update_required =
        aws_ip_update_required || port_number_update_required ||
        serial_number_update_required;
 ...
 */
bool json_parse_stream_on(cJSON *cmd_args, uint32_t *delay_ms,
                          uint32_t *num_frames) {
  int i;
...
/**
 * Get the get_lep_cci arguments.  Pass our cci_buf back to the calling code to
 * hold the read data.
 */
bool json_parse_get_lep_cci(cJSON *cmd_args, uint16_t *cmd, int *len,
                            uint16_t **buf) {
  int i;
  int item_count = 0;
...
/**
 * Get the set_lep_cci arguments.  Fill our cci_buf with register data and pass
 * it back to the calling code.
 */
bool json_parse_set_lep_cci(cJSON *cmd_args, uint16_t *cmd, int *len,
                            uint16_t **buf) {
  char *data;
  int i;
  int item_count = 0;
  ...
  if (cmd_args != NULL) {
    ...
    if (item_count == 2) {
      if (cJSON_HasObjectItem(cmd_args, "data")) {
        ...
        // Decode
        i = mbedtls_base64_decode((unsigned char *)cci_buf, CCI_BUF_LEN,
                                  &dec_len, (const unsigned char *)data,
                                  strlen(data));
        if (i != 0) {
          ESP_LOGE(
              TAG,
              "Base 64 CCI Register data decode failed - %d (%d bytes decoded)",
              i, dec_len);
          return false;
        }
      }
...
bool json_parse_fw_segment(cJSON *cmd_args, uint32_t *start, uint32_t *len,
                           uint8_t *buf) {
  char *data;
  int i;
  int item_count = 0;
  ...
  if (cmd_args != NULL) {
    ...
    if (item_count == 2) {
      if (cJSON_HasObjectItem(cmd_args, "data")) {
        ...
        // Decode
        enc_len = strlen(data);
        i = mbedtls_base64_decode(buf, FM_UPD_CHUNK_MAX_LEN, &dec_len,
                                  (const unsigned char *)data, enc_len);
        if (i != 0) {
          ESP_LOGE(
              TAG,
              "Base 64 FW segment data decode failed - %d (%d bytes decoded)",
              i, dec_len);
          return false;
        }
...
/**
 * Add a child object containing base64 encoded lepton image from the shared
 * buffer
 *
...
static bool json_add_lep_image_object(cJSON *parent, lep_buffer_t *lep_buffer) {
  ...
  if (base64_lep_data != NULL) {
    ...
    ESP_LOGE(TAG, "failed to allocate %d bytes for lepton image base64 text",
             base64_obj_len);
    return false;
  }

  // Add the encoded data as a reference since we're managing the buffer
  cJSON_AddItemToObject(parent, "radiometric",
                        cJSON_CreateStringReference((char *)base64_lep_data));
 ...
 * image json object.
 */
static void json_free_lep_base64_image() { free(base64_lep_data); }

/**
 * Add a child object containing base64 encoded lepton telemetry array from the
 * shared buffer
 *
 * Note: The encoded telemetry string is held in an array that must be freed
 * with json_free_lep_base64_telem() after the json object is converted to a
 * string.
 */
static bool json_add_lep_telem_object(cJSON *parent, lep_buffer_t *lep_buffer) {
  size_t base64_obj_len;
  ...
  if (base64_lep_data != NULL) {
    ...
    if (mbedtls_base64_encode(base64_lep_telem_data, base64_obj_len,
                              &base64_obj_len,
                              (const unsigned char *)lep_buffer->lep_telemP,
                              LEP_TEL_WORDS * 2) != 0) {
    ...
    ESP_LOGE(TAG,
             "failed to allocate %d bytes for lepton telemetry base64 text",
             base64_obj_len);
    return false;
  }
  ...
  cJSON_AddItemToObject(
      parent, "telemetry",
      cJSON_CreateStringReference((char *)base64_lep_telem_data));

  return true;
}

/**
 * Free the base64-encoded Lepton telemetry string.  Call this routine after
 * printing the telemetry json object.
 */
static void json_free_lep_base64_telem() { free(base64_lep_telem_data); }
 ...
 * json_free_cci_reg_base64_data() after the json object is converted to a
 * string.
 */
static bool json_add_cci_reg_base64_data(cJSON *parent, int len,
                                         uint16_t *buf) {
  size_t base64_obj_len;
  ...
  if (base64_cci_reg_data != NULL) {
    ...
    if (mbedtls_base64_encode(base64_cci_reg_data, base64_obj_len,
                              &base64_obj_len, (const unsigned char *)buf,
                              len * 2) != 0) {
    ...
    ESP_LOGE(TAG, "failed to allocate %d bytes for CCI Register base64 text",
             base64_obj_len);
    return false;
  }
  ...
  // Add the encoded data as a reference since we're managing the buffer
  cJSON_AddItemToObject(
      parent, "data", cJSON_CreateStringReference((char *)base64_cci_reg_data));

  return true;
}

/**
 * Free the base64-encoded CCI Register data string.  Call this routine after
 * printing the cci_reg json object.
 */
static void json_free_cci_reg_base64_data() { free(base64_cci_reg_data); }
...
/**
 * Tightly print a response into a string with delimitors for transmission over
 * the network. Returns length of the string.
 */
static uint32_t json_generate_response_string(cJSON *root, char *json_string) {
  uint32_t len;

  json_string[0] = CMD_JSON_STRING_START;
  if (cJSON_PrintPreallocated(root, &json_string[1], JSON_MAX_RSP_TEXT_LEN,
                              false) == 0) {
    len = 0;
  } else {
...
/**
 * Convert a string in the form of "XXX.XXX.XXX.XXX" into a 4-byte array for
 * net_info_t
 */
static bool json_ip_string_to_array(uint8_t *ip_array, char *ip_string) {
  char c;
