...
void aws_cmd_task() {
  ...
  onboarding_details_info.aws_ip_address = aws_ip_adress;
  onboarding_details_info.serial_number = serial_number;
  memset(onboarding_details_info.aws_ip_address, 0, sizeof(aws_ip_adress));
  memset(onboarding_details_info.serial_number, 0, sizeof(serial_number));
  char message[100];
  while (1) {
    if (!(*net_is_connected)()) {
    ...
    ESP_LOGW(TAG,
             ...
             "get_onboarding_details cmd to esp32");
    snprintf(message, sizeof(message), "%c{\"cmd\":\"%s\"}%c",
             CMD_JSON_STRING_START, CMD_GET_ONBOARD_DETAILS_FROM_ESP32_S,
...
esp_websocket_client_handle_t aws_cmd_get_ws_handle() { return ws_client; }
...
void init_aws_client(esp_websocket_client_config_t *config) {
  ESP_LOGE(TAG, "init_aws_client on %s:%d%s", config->host, config->port,
           config->path);
...
esp_websocket_client_config_t get_aws_client_config(char *serialNumber,
                                                    char *host, int port) {
  static char path[100];  // Use a static buffer to avoid dynamic memory
                          // allocation issues
  snprintf(path, sizeof(path), "/mlai/streaming/ws/stream_thermal/%s",
           serialNumber);
...
void start_aws_connection() {
  ...
  while (1) {
    ...
    if (aws_connection_start_code != ESP_OK) {
      ESP_LOGE(TAG, "Error in connecting to websocket: %s",
               esp_err_to_name(aws_connection_start_code));
